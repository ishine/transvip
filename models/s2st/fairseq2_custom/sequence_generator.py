...
from fairseq2_011.typing import Device

from fairseq2_011.generation.beam_search import BeamSearch, StandardBeamSearch
from fairseq2_011.generation.logits_processor import LogitsProcessor
from fairseq2_011.models.encoder_decoder import Seq2SeqDecoder
...
class Seq2SeqGenerator:
    ...
    def __call__(
        ...
        spk_embed: Optional[Tensor] = None,
        ...
        encoder_output, encoder_padding_mask, spk_embed = self._fan_out_encoder_output(
            encoder_output, encoder_padding_mask, spk_embed
        )
        ...
        self._bootstrap_seqs_and_scores(
            seqs, scores, encoder_output, encoder_padding_mask, spk_embed, state_bag
        )
        ...
        for step_nr in range(start_step, max_seq_len - 1):
            ...
            if newly_finished_searches:
                ...
                if spk_embed is not None:
                    spk_embed = spk_embed.unflatten(0, (num_searches, -1))
                    spk_embed = spk_embed[search_indices].flatten(0, 1)
    ...
    def _fan_out_encoder_output(
        self, encoder_output: Tensor, encoder_padding_mask: Optional[Tensor], *args
        ...
        outs_other = [x.index_select(dim=0, index=fan_out_indices) if isinstance(x, torch.Tensor) else None for x in args]

        return encoder_output, encoder_padding_mask, *outs_other
