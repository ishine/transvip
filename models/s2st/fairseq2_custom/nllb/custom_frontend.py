...
@final
class SpeakerEmbeddingFrontend(TransformerFrontend):
    ...
    def __init__(
        ...
        self.spk_embed_idx = spk_embed_idx
        ...
        self.spk_dropout_p = spk_dropout_p
    ...
    def forward(
        ...
        seqs: Tensor,
        seq_lens: Optional[Tensor],
        speaker_embedding: Optional[Tensor],
        state_bag: Optional[IncrementalStateBag] = None,
    ) -> Tuple[Tensor, Optional[Tensor]]:
        ...
        if (not self.training) or random.random() > self.spk_dropout_p:
            if speaker_embedding is not None:   
                for i in range(embeds.shape[0]):
                    embeds[i, seqs[i]==self.spk_embed_idx] = speaker_embedding[i]
